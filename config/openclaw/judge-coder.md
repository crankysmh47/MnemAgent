...
Use MnemAgent user ID `judge-mnemcode`. For WebPort, every repository memory call must set `scope_type` to `repository` and `scope_id` to `crankysmh47/WebPort`. Core preferences use `core/core` only.
