...
test('evidence store captures workspace, tests, diff, memory, and rejects sensitive fields', () => {
  ...
  store.ingest('run_one', { type: 'memory.retrieved', detail: { scope: 'repository/crankysmh47/MnemBench', value: 'Keep 1.0 oriented as best.' } });
  store.ingest('run_one', { type: 'test.completed', detail: { commandId: 'python-unit', exitCode: 0, output: 'all pass' } });
  ...
  assert.equal(evidence.memories[0].scope, 'repository/crankysmh47/MnemBench');
...
test('evidence is ready only with a workspace, non-empty diff, and both required tests passing', () => {
  ...
  store.ingest('run_one', { type: 'test.completed', detail: { commandId: 'python-unit', exitCode: 0, output: 'pass' } });
  assert.equal(store.get('run_one').readyForApproval, false);
  store.ingest('run_one', { type: 'test.completed', detail: { commandId: 'python-scoring-test', exitCode: 0, output: 'pass' } });
...
test('a later passing rerun supersedes an earlier red test for the same command', () => {
  const store = createJudgeEvidenceStore();
  store.ingest('run_one', { type: 'workspace.created', detail: { workspaceId: 'ws_123' } });
  store.ingest('run_one', { type: 'test.completed', detail: { commandId: 'python-unit', exitCode: 1, output: 'red' } });
  store.ingest('run_one', { type: 'test.completed', detail: { commandId: 'python-unit', exitCode: 0, output: 'green' } });
  store.ingest('run_one', { type: 'test.completed', detail: { commandId: 'python-scoring-test', exitCode: 0, output: 'green' } });
  store.ingest('run_one', { type: 'diff.ready', detail: { diff: 'patch' } });
  assert.equal(store.get('run_one').readyForApproval, true);
});
