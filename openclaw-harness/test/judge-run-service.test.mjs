...
test('run service returns immediately, binds ownership, and completes asynchronously', async () => {
  ...
  const run = service.create({ ownerSessionId: 'jss_owner', namespace: 'judge-private', sessionId: 'fresh-session', issueNumber: 14, message: 'Inspect and solve.' });
...
test('hard model token budget switches new runs to replay mode', async () => {
  ...
  const first = service.create({ ownerSessionId: 'jss_one', namespace: 'judge-one', sessionId: 'one', issueNumber: 14, message: 'first' });
  await service.wait(first.id);
  const second = service.create({ ownerSessionId: 'jss_two', namespace: 'judge-two', sessionId: 'two', issueNumber: 14, message: 'second' });
  await service.wait(second.id);
  assert.equal(second.mode, 'replay');
  assert.equal(service.get(second.id, 'jss_two').status, 'completed');
});

test('coding runs accept only prepared issue 14 and bounded directions', () => {
  ...
  assert.throws(() => service.create({ ownerSessionId: 'jss_owner', namespace: 'judge-private', sessionId: 'fresh', issueNumber: 14, message: 'x'.repeat(4_001) }), /bounded/i);
});
