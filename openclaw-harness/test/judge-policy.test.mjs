...
test('judge runs allow only prepared issues and bounded messages', () => {
  assert.deepEqual(validateJudgeRun({ issueNumber: 14, message: 'Add the missing operand guard.' }), { issueNumber: 14, message: 'Add the missing operand guard.' });
  assert.throws(() => validateJudgeRun({ issueNumber: 99, message: 'x' }), /prepared issue/);
  assert.throws(() => validateJudgeRun({ issueNumber: 14, message: 'x'.repeat(2001) }), /2,000/);
});
