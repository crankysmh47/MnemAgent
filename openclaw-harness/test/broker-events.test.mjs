...
test('broker events expose bounded evidence without source or patch payloads', () => {
  assert.deepEqual(projectBrokerEvent('create_workspace', { repository: 'crankysmh47/MnemBench', issueNumber: 1 }, { id: 'ws_1', branch: 'judge/1' }), { type: 'workspace.created', detail: { workspaceId: 'ws_1', branch: 'judge/1' } });
  ...
  assert.deepEqual(projectBrokerEvent('run_workspace_tests', { commandId: 'python-unit' }, { exitCode: 0, output: 'pass' }), { type: 'test.completed', detail: { commandId: 'python-unit', exitCode: 0, output: 'pass' } });
