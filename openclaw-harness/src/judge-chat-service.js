...
function createJudgeChatService({ model = process.env.JUDGE_MODEL || 'deepseek-api/deepseek-v4-flash', executor = openClawExecutor } = {}) {
  ...
  return {
    create({ ownerSessionId, namespace, message }) {
      ...
      turns.set(id, turn);
      const contractMessage = `This is a conversational memory turn for private user ${namespace}. Reply naturally and search existing memories before answering. When the user explicitly asks you to remember a durable preference, decision, correction, or repository rule, you must call memory_store before replying. Otherwise, call memory_store only when the current message clearly introduces or corrects durable information. A question that merely asks you to recall must never write another memory. If the message is about MnemBench or a MnemBench convention, use scope_type repository and scope_id crankysmh47/MnemBench. Use core/core only for preferences that apply across repositories. Do not use repository workspace tools. User message: ${message}`;
      turn.task = (async () => {
        try {
