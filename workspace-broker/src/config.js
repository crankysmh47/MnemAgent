...
export function loadConfig(env = process.env) {
  const config = {
    ...
    workspaceVolume: String(env.WORKSPACE_VOLUME || 'mnemagent-judge-workspaces'),
  ...
  if (!/^[A-Za-z0-9_.-]+$/.test(config.workspaceVolume)) throw new Error('WORKSPACE_VOLUME is invalid.');
  return config;
}
