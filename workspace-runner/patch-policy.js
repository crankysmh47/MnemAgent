...
export function validatePatch(patch = '') {
  ...
  for (const file of files) {
    if (file.includes('..') || file.startsWith('/') || PROTECTED.test(file) || !/^(?:(?:src|include|test|tests|public|tools)\/[A-Za-z0-9_./-]+\.(?:js|mjs|cjs|cpp|cc|c|h|hpp|css|html|json|py)|mnembench\/[A-Za-z0-9_./-]+\.py)$/.test(file)) throw new Error(`Patch path is not allowed: ${file}`);
  }
